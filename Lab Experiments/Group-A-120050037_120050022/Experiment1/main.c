...
* Edited by: Saurav Shandilya, Vishwanathan Iyer 
...
int main(void)
{
	...
	ledPinConfig();
	switchPinConfig();
	uint8_t led = 2; //intializing the led color as red
	...
	while(1) {
		...
		if(GPIOPinRead(GPIO_PORTF_BASE,GPIO_PIN_4)==0x00) {
			...
			if (!pressed) {
				//led colors get changed in the order: red, blue, green, back to red and so on
				led = led*2;
				if (led == 16) led = 2;
				pressed = true;
			}
		...
		if(GPIOPinRead(GPIO_PORTF_BASE,GPIO_PIN_0)==0x00) {
			if (!pressed2) {
				sw2Status = sw2Status + 1; // increment the counter
				pressed2 = true;
			}
		}
		else {
