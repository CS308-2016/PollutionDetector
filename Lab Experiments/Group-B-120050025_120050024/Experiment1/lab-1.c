...
int main(void)
{
	...
	while(1)
	{
		if(GPIOPinRead(GPIO_PORTF_BASE,GPIO_PIN_4)==0x00)//sw1 pressed
		{
			...
			while(GPIOPinRead(GPIO_PORTF_BASE,GPIO_PIN_4)==0x00);//while switch is kept pressed do nothing
			GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3, 0);
		}


		if(GPIOPinRead(GPIO_PORTF_BASE,GPIO_PIN_0)==0x00)//sw2 pressed
		{
			sw2Status ++;
